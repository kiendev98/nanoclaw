...
| Tool | What it does |
|------|-------------|
| `create_worker` | `action: 'create_worker'` (repo + task); host resolves the repo, creates or reuses the worktree and agent group, and delivers the brief. Blocking, bounded at 60s. There is no `create_agent` MCP tool — the host-side action of that name is driven by the operator and by `slack-agent-flow` |
