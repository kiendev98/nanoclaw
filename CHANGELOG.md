...
- **An agent can hand work to a worker inside another repository.** An assistant cannot stand in two repositories at once, because each carries its own instructions, tools and conventions — so `delegate_task` creates a worker whose working directory is a git worktree of the named repository, and the target repository's own `CLAUDE.md`, skills and settings load through the provider's ordinary walk. Three keys, not one: the agent group is per repository, so memory is shared across its threads; the session is per (repository, messaging group, thread); the worktree is per session, because two threads working one repository at once must not share a working copy. Exactly one report reaches the assistant per task, and the HOST sends it — `finish_task` is a fast path, and the session-terminal hook is the backstop that fires however a run ended, so a worker that crashes still reports how far it got. The worker gets no destination row of any kind, so its only routes back are `ask_principal`, `send_progress_note` and that one report. `lend_conversation` gives a worker one thread of its own for a review loop, released when the task ends. Repositories come from `NANOCLAW_PROJECT_ROOTS` (a `:`-separated list of parent folders, git checkouts one level inside); refusals name repositories, never host paths. Installs that set nothing are unaffected: with no configured folder, `delegate_task` refuses and nothing else changes.
- **Chat SDK and channel adapters move to `4.39.0`, and the agent SDKs to their current releases.** The bridge and adapter must carry the same `ChatInstance` type, so the pin is lockstep: `chat`, `@chat-adapter/slack`, and the eight `/add-<channel>` skills all name one version. None of the breaking changes between 4.30 and 4.39 reach this code — `createChatTools` and `markAsRead` are unused, and the 4.37 read-scoping rejects only a handler that reads another channel, while the one handler here (`onMemberJoinedChannel`) forwards event fields and reads none. `@slack/web-api` stays on 7.x, because 4.39.0 still asks for `^7.18.0`. In the container, `@anthropic-ai/claude-agent-sdk` goes to `0.3.259` — pinned exactly rather than with a caret, since that tree has no `minimumReleaseAge` gate and a range silently took the release published hours earlier — plus `@modelcontextprotocol/sdk` `1.30.0` (fixes only) and `@anthropic-ai/sdk` `0.123.0`, which has no import site at all and exists to satisfy a peer range. **Migration:** if a channel is installed, re-run its `/add-<channel>` skill after updating.
