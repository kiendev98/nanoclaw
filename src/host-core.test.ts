...
/**
 * A repo worker has no channel of its own — its session is `agent-shared`
 * with no messaging group — so anything it wrote as ordinary output used to
 * go NOWHERE, silently. Its reply lane is therefore set by CODE, from
 * `agent_groups.origin_session_id`, and `delivery.ts` already routes
 * `channel_type === 'agent'` into `routeAgentMessage`.
 *
 * The lane is fixed at creation and is not a wider permission: the route
 * still passes the `a2a.send` guard, which denies any pair with no
 * `agent_destinations` row.
 */
describe('writeSessionRouting — repo worker reply lane', () => {
  const WORKTREE = '/tmp/nanoclaw-test-host/worktrees/demo-nanoclaw-orch';

  async function wireWorker(): Promise<{ orchSessionId: string; workerSessionId: string }> {
    await createAgentGroup({
      id: 'ag-orch',
      name: 'Orchestrator',
      folder: 'orch',
    ...
    const { session: orchSession } = await resolveSession('ag-orch', 'mg-orch', null, 'shared');
    ...
    await createAgentGroup({
      ...
      origin_session_id: orchSession.id,
    });
    // Both destination rows, exactly as provisioning writes them. The
    // worker's row for its orchestrator is what the a2a guard consults.
    const db = getDb();
    await db.run(
      `INSERT INTO agent_destinations (agent_group_id, local_name, target_type, target_id, created_at)
       VALUES ('ag-orch', 'scout', 'agent', 'ag-worker', ?)`,
      now(),
    );
    await db.run(
      `INSERT INTO agent_destinations (agent_group_id, local_name, target_type, target_id, created_at)
       VALUES ('ag-worker', 'parent', 'agent', 'ag-orch', ?)`,
      now(),
    );

    const { session: workerSession } = await resolveSession('ag-worker', null, null, 'agent-shared');
    return { orchSessionId: orchSession.id, workerSessionId: workerSession.id };
  }

  function routingOf(agentGroupId: string, sessionId: string) {
  ...
  it('resolves a worker session to the agent lane addressed at its orchestrator', async () => {
    const { workerSessionId } = await wireWorker();

    await writeSessionRouting('ag-worker', workerSessionId);

    const routing = routingOf('ag-worker', workerSessionId);
    expect(routing!.channel_type).toBe('agent');
    expect(routing!.platform_id).toBe('ag-orch');
    // An agent-to-agent message has no thread.
    expect(routing!.thread_id).toBeNull();
  });

  it('leaves a session WITH a messaging group exactly as it was', async () => {
    // Only a channel-less worker gets the lane. An agent wired to a chat must
    // keep answering into that chat, worktree or not.
    const { orchSessionId } = await wireWorker();

    await writeSessionRouting('ag-orch', orchSessionId);

    const routing = routingOf('ag-orch', orchSessionId);
    expect(routing!.channel_type).toBe('slack');
    expect(routing!.platform_id).toBe('C-ORCH');
  });

  it("delivers a worker's plain output to the orchestrator's mailbox with no send_message call", async () => {
    // What the runner writes for an unwrapped turn: a chat row addressed with
    // the session's own routing. Nothing the worker did named a destination.
    const { orchSessionId, workerSessionId } = await wireWorker();
    await writeSessionRouting('ag-worker', workerSessionId);
    const routing = routingOf('ag-worker', workerSessionId);

    const { routeAgentMessage } = await import('./modules/agent-to-agent/agent-route.js');
    const workerSession = await getSession(workerSessionId);
    await routeAgentMessage(
      {
        id: 'worker-turn-1',
        platform_id: routing!.platform_id,
        content: JSON.stringify({ text: 'The gates pass. Three files changed.' }),
        in_reply_to: null,
      },
      workerSession!,
    );

    const db = new Database(inboundDbPath('ag-orch', orchSessionId));
    const rows = db
      .prepare("SELECT platform_id, content FROM messages_in WHERE channel_type = 'agent'")
      .all() as Array<{
      platform_id: string;
      content: string;
    }>;
    ...
    expect(rows).toHaveLength(1);
    expect(rows[0].platform_id).toBe('ag-worker');
    expect(JSON.parse(rows[0].content).text).toBe('The gates pass. Three files changed.');
  });

  it('refuses to route a worker anywhere but its own orchestrator', async () => {
    // The lane is fixed at creation, and the destination map is the second
    // lock: a worker holds one row, for its orchestrator, and nothing it
    // emits can address a third group.
    const { workerSessionId } = await wireWorker();
    await createAgentGroup({
      id: 'ag-stranger',
      name: 'Stranger',
      ...
      routeAgentMessage(
        {
          ...
          content: JSON.stringify({ text: 'psst' }),
          in_reply_to: null,
        },
        workerSession!,
      ),
