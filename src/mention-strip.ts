 ...
 *
 * DUPLICATED ON PURPOSE. The container-side copy is
 * `container/agent-runner/src/mention-strip.ts`, and the two must stay
 * identical. They cannot import one another: the host is compiled by `tsc` with
 * `rootDir: ./src` and the agent runner is a separate build unit run by Bun
 * with `rootDir: ./container/agent-runner/src`, so nothing crosses that line —
 * the mailbox model is copied through `npm run mailbox-model:generate` for the
 * same reason. Change one, change the other.
 */
