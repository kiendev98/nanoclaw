 ...
 * create_room is the team primitive: one call names every agent that should
 * share the conversation. Both tools are fire-and-forget — they write an
 ...
 *
 * Authorization is enforced host-side by the guard (trusted global-scope
 * groups act directly, confined groups hold for admin approval) — the container is untrusted and cannot be relied
 * on to gate itself.
 *
 ...
 * so those three keys now reach it from a host caller, never from a tool.
 */
